...
export async function deleteUser(req, res) {
  ...
  try {
    ...
    // Fetch all users that are NOT soft deleted (handled automatically)
    const users = await User.findAll(); // No need for is_deleted check
...
export async function newUser(req, res) {
  ...
  try {
    ...
    const existing = await User.findOne({ where: { email }, paranoid: false }); // reuse soft-deletd email
